...
local function SuppressCustomAuras(element)
...
local function UpdateCustomAuras(self, event, unit, updateInfo)
	...
	-- 12.1: UNIT_AURA puede ser completamente secret. CustomAuras es
	-- arquitectura legacy y no debe intentar inspeccionarlo.
	if updateInfo and IsSecret(updateInfo) then
		SuppressCustomAuras(element)
		return
	end
	...
	local isFullUpdate = element.needFullUpdate or not updateInfo or fullUpdateFlag
	...
	* isFullUpdate - indicates whether the element is performing a full update (boolean)
	...
	if(isFullUpdate) then
		...
		local slots = SafeGetAuraSlots(unit, filter)
		if not slots then
			SuppressCustomAuras(element)
			return
		end
		...
		for i = 2, #slots do
			...
			local data = processData(element, unit, rawData, filter)
			if(data) then
		...
		if(updateInfo.updatedAuraInstanceIDs) then
			for _, auraInstanceID in next, updateInfo.updatedAuraInstanceIDs do
				if(element.all[auraInstanceID]) then
					...
					element.all[auraInstanceID] = processData(element, unit, rawData, filter)
