...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	local isVisible = SafeBoolean(UnitIsVisible(unit))

	-- Si Blizzard protege alguno de estos booleanos no podemos combinarlos
	-- con "and/not". Como fallback dejamos que SetUnit() resuelva el retrato.
	local isAvailable
	if(isConnected == false or isVisible == false) then
		isAvailable = false
	else
		isAvailable = true
	end
	...
	if(hasStateChanged) then
		if(element:IsObjectType('PlayerModel')) then
			...
			if(class and not issecretvalue(class)) then
				...
				-- Si la clase está protegida, usamos el retrato normal en lugar
				-- de concatenar un secret con "classicon-".
