...
local function SafeGetAuraSlots(unit, filter)
...
local function SuppressLegacyAuraElement(element)
...
local function SuppressLegacyAuras(self)
	SuppressLegacyAuraElement(self.Auras)
	SuppressLegacyAuraElement(self.Buffs)
	SuppressLegacyAuraElement(self.Debuffs)
end

local function UpdateAuras(self, event, unit, updateInfo)
	if(self.unit ~= unit) then return end

	-- Midnight 12.1: UUF muestra target/boss mediante AuraContainer.
	-- Bloqueo explícito del camino legacy para no tocar GetAuraSlots()
	-- cuando las auras son secret.
	if self.UUFManagedAuraMode
		or unit == "target"
	...
	-- 12.1: UNIT_AURA puede entregar un payload completamente secret.
	-- No se puede indexar ni evaluar desde código tainted.
	if updateInfo and IsSecret(updateInfo) then
		SuppressLegacyAuras(self)
		return
	end

	local fullUpdateFlag = updateInfo and updateInfo.isFullUpdate
	if IsSecret(fullUpdateFlag) then
		SuppressLegacyAuras(self)
		return
	end

	local isFullUpdate = not updateInfo or fullUpdateFlag
	...
	if(auras) then
		...
		* isFullUpdate - indicates whether the element is performing a full update (boolean)
		...
		if(isFullUpdate) then
			...
			local slots = SafeGetAuraSlots(unit, buffFilter)
			if not slots then
				SuppressLegacyAuras(self)
				return
			end
			for i = 2, #slots do -- #1 return is continuationToken, we don't care about it
				local rawData = SafeGetAuraDataBySlot(unit, slots[i])
				if not rawData then
					SuppressLegacyAuras(self)
					return
				end
				local data = processData(auras, unit, rawData, buffFilter)
			...
			slots = SafeGetAuraSlots(unit, debuffFilter)
			if not slots then
				SuppressLegacyAuras(self)
				return
			end
			for i = 2, #slots do
				...
				local data = processData(auras, unit, rawData, debuffFilter)
			...
			if(updateInfo.updatedAuraInstanceIDs) then
				for _, auraInstanceID in next, updateInfo.updatedAuraInstanceIDs do
					if(auras.allBuffs[auraInstanceID]) then
						local rawData = SafeGetAuraDataByInstanceID(unit, auraInstanceID)
						if not rawData then
							SuppressLegacyAuras(self)
							return
						end
						auras.allBuffs[auraInstanceID] = processData(auras, unit, rawData, buffFilter)
					...
					elseif(auras.allDebuffs[auraInstanceID]) then
						...
						auras.allDebuffs[auraInstanceID] = processData(auras, unit, rawData, debuffFilter)
	...
	if(buffs) then
		...
		if(isFullUpdate) then
			...
			for i = 2, #slots do
				...
				local data = processData(buffs, unit, rawData, buffFilter)
			...
			if(updateInfo.updatedAuraInstanceIDs) then
				for _, auraInstanceID in next, updateInfo.updatedAuraInstanceIDs do
					if(buffs.all[auraInstanceID]) then
						...
						buffs.all[auraInstanceID] = processData(buffs, unit, rawData, buffFilter)
	...
	if(debuffs) then
		...
		if(isFullUpdate) then
			...
			local slots = SafeGetAuraSlots(unit, debuffFilter)
			if not slots then
				SuppressLegacyAuras(self)
				return
			end
			for i = 2, #slots do
				...
				local data = processData(debuffs, unit, rawData, debuffFilter)
			...
			if(updateInfo.updatedAuraInstanceIDs) then
				for _, auraInstanceID in next, updateInfo.updatedAuraInstanceIDs do
					if(debuffs.all[auraInstanceID]) then
						...
						debuffs.all[auraInstanceID] = processData(debuffs, unit, rawData, debuffFilter)
