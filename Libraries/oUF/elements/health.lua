...
local function UpdateColor(self, event, unit)
	...
	-- Midnight: several unit APIs can return secret values in combat.
	-- Never use those values as table keys or boolean-test them in addon Lua.
	...
	if(not color and element.colorTapping and isPlayerControlled == false and isTapDenied == true) then
		color = self.colors.tapped
	end

	if(not color and element.colorThreat and isPlayerControlled == false and threat ~= nil) then
	...
	if(not color and element.colorSmooth and self.colors.health:GetCurve()) then
