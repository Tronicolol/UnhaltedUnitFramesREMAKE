...
local tagStrings = {
	...
	['leader'] = [[function(u)
		...
		if(issecretvalue(isLeader)) then return end
		if(isLeader) then
			return 'L'
		end
	end]],
	...
	['powercolor'] = [[function(u)
		...
		local color = pToken and _COLORS.power[pToken]

		if(not color) then
			local hasAlt = altR ~= nil and altG ~= nil and altB ~= nil

			if(hasAlt and not issecretvalue(altR) and not issecretvalue(altG) and not issecretvalue(altB)) then
			...
			-- Si Blizzard protege los RGB alternativos no hacemos
			-- comparaciones ni aritmética con ellos.
	...
	['pvp'] = [[function(u)
		...
		if(issecretvalue(isPVP)) then return end
		if(isPVP) then
			return 'PvP'
		end
	end]],
	...
	['raidcolor'] = [[function(u)
		...
		-- 12.1: UnitClass puede ser un string secret y no puede usarse
		-- como clave de _COLORS.class.
		if(class and not issecretvalue(class)) then
			local classColor = _COLORS.class[class]
			return classColor and classColor:GenerateHexColorMarkup()
		end
	...
	['sex'] = [[function(u)
		local s = UnitSex(u)
		if(issecretvalue(s)) then return end
		if(s == 2) then
			return 'Male'
		elseif(s == 3) then
			return 'Female'
	...
	['threatcolor'] = [[function(u)
		...
		local color = _COLORS.threat[status or 0]
		return color and color:GenerateHexColorMarkup()
	end]],
}
