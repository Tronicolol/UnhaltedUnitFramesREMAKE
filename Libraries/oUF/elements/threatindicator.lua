...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	-- Midnight: UnitThreatSituation() puede devolver un número secret
	-- para determinadas combinaciones de unit tokens. Un addon tainted
	-- no puede hacer comparaciones como "status > 0" con ese valor.
	if(issecretvalue and issecretvalue(status)) then
		element:Hide()

		-- No propagamos el secret al callback personalizado de UUF, que
		-- también realiza comparaciones numéricas con status.
		if(element.PostUpdate) then
			return element:PostUpdate(unit, nil, nil)
		end
		return
	end
