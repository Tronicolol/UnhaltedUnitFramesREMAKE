...
-- UUF Party/Raid optimization: allow the layout to suppress UNIT_HEAL_PREDICTION
-- when no incoming-heal widget is enabled. Absorb/heal-absorb updates continue
-- to use their dedicated unit events. Other layouts keep the stock behavior.
local function SetUUFTrackHealPredictionEvent(element, enabled)
	local owner = element and element.__owner
	if(not owner) then
		element.UUFTrackHealPredictionEvent = enabled ~= false
		return
	end

	element.UUFTrackHealPredictionEvent = enabled ~= false
	if(element.UUFTrackHealPredictionEvent) then
		owner:RegisterEvent('UNIT_HEAL_PREDICTION', Path)
	else
		owner:UnregisterEvent('UNIT_HEAL_PREDICTION', Path)
	end
end
...
local function Enable(self)
	...
	if(element) then
		...
		element.SetUUFTrackHealPredictionEvent = SetUUFTrackHealPredictionEvent
		...
		if(element.UUFTrackHealPredictionEvent ~= false) then
