...
function Private.unitSelectionType(unit, considerHostile)
	if(considerHostile) then
		...
		-- Midnight 12.1: threat puede ser un número secret.
		-- No puede evaluarse como condición Lua.
		if(not issecretvalue(threat) and threat) then
			return 0
		end
	end
	...
	-- Un valor secret no puede utilizarse como clave de tabla.
	if(selectionType == nil or issecretvalue(selectionType)) then
		return nil
	end

	return validSelectionTypes[selectionType]
end
