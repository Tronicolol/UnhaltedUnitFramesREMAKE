...
local function RelayoutGUIParents(widget)
	local current = widget
	local depth = 0
	while current and depth < 16 do
		if current.DoLayout then current:DoLayout() end
		if current.FixScroll then current:FixScroll() end
...
local function CreateFrameSettings(containerParent, unit, unitHasParent, updateCallback)
    ...
    if unit == "party" then
        ...
        local ShowSoloPartyToggle = AG:Create("CheckBox")
        ShowSoloPartyToggle:SetLabel("Show Party Frame While Solo")
        ...
        ShowSoloPartyToggle:SetCallback("OnValueChanged", function(_, _, value)
            ...
            StaticPopup_Show("UUF_RELOAD_UI_SOLO_PARTY")
        end)
        ShowSoloPartyToggle:SetCallback("OnEnter", function()
            GameTooltip:SetOwner(ShowSoloPartyToggle.frame, "ANCHOR_CURSOR")
            GameTooltip:AddLine("Shows one real Party-style frame for your own character while you are not in a group. It uses the real player unit, so Party buffs/debuffs and aura filters can be tested while solo.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
        ShowSoloPartyToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
        ColourContainer:AddChild(ShowSoloPartyToggle)
    end
...
local function CreateSpecificAuraSettings(containerParent, unit, auraDB)
    ...
    local isPartyRaidAura = unit == "party" or unit == "raid"
    local isBossDebuffs = unit == "boss" and auraDB == "Debuffs"
    local isEnemyDebuffs = auraDB == "Debuffs" and (unit == "target" or unit == "targettarget" or unit == "focus" or unit == "focustarget")
    -- The managed 12.1 filter engine is shared by every non-boss unit frame.
    -- Boss Debuffs intentionally retain their dedicated source/whitelist model.
    local isUnifiedManagedAura = unit == "player"
        or unit == "target"
    ...
    Toggle:SetRelativeWidth(isCustom and 0.5 or ((isUnifiedManagedAura or isBossDebuffs) and 0.5 or 0.33))
    ...
    -- Party/Raid source selection is handled by the Player/Others dropdowns.
    -- Keep the legacy shortcut only on the other unit frames.
    if not isUnifiedManagedAura and not isBossDebuffs then
        ...
        OnlyShowPlayerToggle:SetCallback("OnEnter", function()
            GameTooltip:SetOwner(OnlyShowPlayerToggle.frame, "ANCHOR_CURSOR")
            GameTooltip:AddLine("Overrides |cFF8080FF" .. auraTitle:lower() .. "|r advanced filters.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
    ...
    ShowTypeCheckbox:SetRelativeWidth(isCustom and (isUnifiedManagedAura and 1 or 0.5) or ((isUnifiedManagedAura or isBossDebuffs) and 0.5 or 0.33))
    AuraContainer:AddChild(ShowTypeCheckbox)

    local PandemicGlowContainer
    local BossDebuffFilterContainer
    local PandemicToggle
    local PandemicTypeDropdown
    local PandemicColourPicker
    local OnlyMyDebuffsToggle
    local OnlyWhitelistToggle
    local WhitelistEditBox
    local BlacklistEditBox
    local PandemicDB
    local BossDebuffFilterDB

    if isBossDebuffs then
        AuraDB.PandemicGlow = AuraDB.PandemicGlow or {}
        PandemicDB = AuraDB.PandemicGlow
        if PandemicDB.Enabled == nil then PandemicDB.Enabled = false end
        ...
        PandemicDB.Type = legacyTypes[PandemicDB.Type] or PandemicDB.Type or "BUTTON"
        if type(PandemicDB.Colour) ~= "table" then PandemicDB.Colour = {1, 0.82, 0, 1} end

        AuraDB.BossDebuffFilter = AuraDB.BossDebuffFilter or {}
        BossDebuffFilterDB = AuraDB.BossDebuffFilter
        if BossDebuffFilterDB.OnlyMyDebuffs == nil then
            BossDebuffFilterDB.OnlyMyDebuffs = false
        end
        if BossDebuffFilterDB.OnlyWhitelist == nil then
            BossDebuffFilterDB.OnlyWhitelist = AuraDB.PandemicGlow.OnlyWhitelist == true
        end
        if BossDebuffFilterDB.Whitelist == nil then
            BossDebuffFilterDB.Whitelist = AuraDB.PandemicGlow.Whitelist or ""
        end
        if BossDebuffFilterDB.Blacklist == nil then
            BossDebuffFilterDB.Blacklist = AuraDB.PandemicGlow.Blacklist or ""
        end
        if type(BossDebuffFilterDB.Whitelist) ~= "string" then BossDebuffFilterDB.Whitelist = "" end
        if type(BossDebuffFilterDB.Blacklist) ~= "string" then BossDebuffFilterDB.Blacklist = "" end

        -- Boss filtering is deliberately source-focused.
        BossDebuffFilterContainer = GUIWidgets.CreateInlineGroup(containerParent, "Boss Debuff Filters")

        OnlyMyDebuffsToggle = AG:Create("CheckBox")
        OnlyMyDebuffsToggle:SetLabel("Only |cFF8080FFMy Debuffs|r")
        OnlyMyDebuffsToggle:SetValue(BossDebuffFilterDB.OnlyMyDebuffs)
        OnlyMyDebuffsToggle:SetRelativeWidth(1)
        OnlyMyDebuffsToggle:SetCallback("OnValueChanged", function(_, _, value)
            BossDebuffFilterDB.OnlyMyDebuffs = value
            UpdateAuras()
            RefreshAuraGUI()
        end)
        OnlyMyDebuffsToggle:SetCallback("OnEnter", function()
            GameTooltip:SetOwner(OnlyMyDebuffsToggle.frame, "ANCHOR_CURSOR")
            GameTooltip:AddLine("Enabled: every displayed Boss debuff must be applied by you. This restriction also applies to Whitelist. If Only Whitelist is also enabled, only your whitelisted debuffs are shown. Blacklist can still hide specific Debuff IDs.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
        OnlyMyDebuffsToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
        BossDebuffFilterContainer:AddChild(OnlyMyDebuffsToggle)

        OnlyWhitelistToggle = AG:Create("CheckBox")
        OnlyWhitelistToggle:SetLabel("Only |cFF8080FFWhitelist|r")
        OnlyWhitelistToggle:SetValue(BossDebuffFilterDB.OnlyWhitelist)
        OnlyWhitelistToggle:SetRelativeWidth(1)
        OnlyWhitelistToggle:SetCallback("OnValueChanged", function(_, _, value)
            BossDebuffFilterDB.OnlyWhitelist = value
            UpdateAuras()
            RefreshAuraGUI()
        end)
        OnlyWhitelistToggle:SetCallback("OnEnter", function()
            GameTooltip:SetOwner(OnlyWhitelistToggle.frame, "ANCHOR_CURSOR")
            GameTooltip:AddLine("Enabled: show only Debuff IDs listed in the Whitelist. If Only My Debuffs is also enabled, the debuff must satisfy both rules: it must be yours and whitelisted.\nBlacklist always wins.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
        OnlyWhitelistToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
        BossDebuffFilterContainer:AddChild(OnlyWhitelistToggle)

        WhitelistEditBox = AG:Create("MultiLineEditBox")
        WhitelistEditBox:SetLabel("Whitelist Debuff IDs")
        WhitelistEditBox:SetText(BossDebuffFilterDB.Whitelist)
        WhitelistEditBox:SetNumLines(3)
        WhitelistEditBox:SetRelativeWidth(0.5)
        WhitelistEditBox:SetCallback("OnEnterPressed", function(_, _, value) BossDebuffFilterDB.Whitelist = value or "" UpdateAuras() end)
        BossDebuffFilterContainer:AddChild(WhitelistEditBox)

        BlacklistEditBox = AG:Create("MultiLineEditBox")
        BlacklistEditBox:SetLabel("Blacklist Debuff IDs")
        BlacklistEditBox:SetText(BossDebuffFilterDB.Blacklist)
        BlacklistEditBox:SetNumLines(3)
        BlacklistEditBox:SetRelativeWidth(0.5)
        BlacklistEditBox:SetCallback("OnEnterPressed", function(_, _, value) BossDebuffFilterDB.Blacklist = value or "" UpdateAuras() end)
        BossDebuffFilterContainer:AddChild(BlacklistEditBox)

        GUIWidgets.CreateInformationTag(BossDebuffFilterContainer, "|cFF8080FFOnly My Debuffs|r restricts every result to your own debuffs. |cFF8080FFOnly Whitelist|r restricts results to listed IDs. With both enabled, only your whitelisted debuffs are shown. With both off: your debuffs + Whitelist exceptions. |cFF8080FFBlacklist|r always wins.")

        -- Keep the proven Pandemic implementation untouched; only its GUI
        -- position moves below the manual Boss filters.
        PandemicGlowContainer = GUIWidgets.CreateInlineGroup(containerParent, "Pandemic Glow Settings")

        PandemicToggle = AG:Create("CheckBox")
        PandemicToggle:SetLabel("Enable |cFF8080FFPandemic Glow|r")
        PandemicToggle:SetValue(PandemicDB.Enabled)
        PandemicToggle:SetRelativeWidth(0.33)
        PandemicToggle:SetCallback("OnValueChanged", function(_, _, value) PandemicDB.Enabled = value UpdateAuras() RefreshAuraGUI() end)
        PandemicToggle:SetCallback("OnEnter", function() GameTooltip:SetOwner(PandemicToggle.frame, "ANCHOR_CURSOR") GameTooltip:AddLine("Shows the selected glow on displayed Boss debuffs while Blizzard marks them inside the Pandemic refresh window.", 1, 1, 1, true) GameTooltip:Show() end)
        PandemicToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
        PandemicGlowContainer:AddChild(PandemicToggle)

        PandemicTypeDropdown = AG:Create("Dropdown")
        PandemicTypeDropdown:SetList({
            ["PIXEL"] = "Pixel Glow",
            ["AUTOCAST"] = "Autocast Glow",
            ["BUTTON"] = "Button Glow",
            ["PROC"] = "Proc Glow",
        }, {"PIXEL", "AUTOCAST", "BUTTON", "PROC"})
        PandemicTypeDropdown:SetLabel("Glow Type")
        PandemicTypeDropdown:SetValue(PandemicDB.Type)
        PandemicTypeDropdown:SetRelativeWidth(0.33)
        PandemicTypeDropdown:SetCallback("OnValueChanged", function(_, _, value) PandemicDB.Type = value UpdateAuras() end)
        PandemicGlowContainer:AddChild(PandemicTypeDropdown)

        PandemicColourPicker = AG:Create("ColorPicker")
        PandemicColourPicker:SetLabel("Glow Colour")
        PandemicColourPicker:SetColor(unpack(PandemicDB.Colour))
        PandemicColourPicker:SetHasAlpha(true)
        PandemicColourPicker:SetRelativeWidth(0.34)
        PandemicColourPicker:SetCallback("OnValueChanged", function(_, _, r, g, b, a) PandemicDB.Colour = {r, g, b, a} UpdateAuras() end)
        PandemicGlowContainer:AddChild(PandemicColourPicker)
    end

    local FilterContainer
    local CategoryFiltersToggle
    local PartyRaidWhitelistEditBox
    local PartyRaidBlacklistEditBox
    local EnemyOnlyMyDebuffsToggle
    local EnemyOnlyWhitelistToggle
    local EnemyDebuffFilterDB
    local FilterDropdowns = {}
    local GeneralFilterToggles = {}

    if not isBossDebuffs then
        FilterContainer = GUIWidgets.CreateInlineGroup(containerParent, auraTitle .. " Filters")
        AuraDB.Filters = AuraDB.Filters or {}

    if isUnifiedManagedAura then
        ...
        local auraIDName = filterAuraDB == "Debuffs" and "Debuff IDs" or "Buff IDs"

        if isEnemyDebuffs then
            AuraDB.EnemyDebuffFilter = AuraDB.EnemyDebuffFilter or {}
            EnemyDebuffFilterDB = AuraDB.EnemyDebuffFilter
            if EnemyDebuffFilterDB.OnlyMyDebuffs == nil then EnemyDebuffFilterDB.OnlyMyDebuffs = true end
            if EnemyDebuffFilterDB.OnlyWhitelist == nil then EnemyDebuffFilterDB.OnlyWhitelist = false end

            EnemyOnlyMyDebuffsToggle = AG:Create("CheckBox")
            EnemyOnlyMyDebuffsToggle:SetLabel("Only |cFF8080FFMy Debuffs|r")
            EnemyOnlyMyDebuffsToggle:SetValue(EnemyDebuffFilterDB.OnlyMyDebuffs == true)
            EnemyOnlyMyDebuffsToggle:SetRelativeWidth(0.5)
            EnemyOnlyMyDebuffsToggle:SetCallback("OnValueChanged", function(_, _, value)
                EnemyDebuffFilterDB.OnlyMyDebuffs = value == true
                UpdateAuras()
                RefreshAuraGUI()
            end)
            EnemyOnlyMyDebuffsToggle:SetCallback("OnEnter", function()
                GameTooltip:SetOwner(EnemyOnlyMyDebuffsToggle.frame, "ANCHOR_CURSOR")
                GameTooltip:AddLine("Enabled: every displayed debuff must be applied by you. Whitelist cannot bypass this source restriction.\nWith Only Whitelist also enabled, only your whitelisted debuffs are shown.", 1, 1, 1, true)
                GameTooltip:Show()
            end)
            EnemyOnlyMyDebuffsToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
            FilterContainer:AddChild(EnemyOnlyMyDebuffsToggle)

            EnemyOnlyWhitelistToggle = AG:Create("CheckBox")
            EnemyOnlyWhitelistToggle:SetLabel("Only |cFF8080FFWhitelist|r")
            EnemyOnlyWhitelistToggle:SetValue(EnemyDebuffFilterDB.OnlyWhitelist == true)
            EnemyOnlyWhitelistToggle:SetRelativeWidth(0.5)
            EnemyOnlyWhitelistToggle:SetCallback("OnValueChanged", function(_, _, value)
                EnemyDebuffFilterDB.OnlyWhitelist = value == true
                UpdateAuras()
                RefreshAuraGUI()
            end)
            EnemyOnlyWhitelistToggle:SetCallback("OnEnter", function()
                GameTooltip:SetOwner(EnemyOnlyWhitelistToggle.frame, "ANCHOR_CURSOR")
                GameTooltip:AddLine("Enabled: show only Debuff IDs listed in Whitelist.\nWith Only My Debuffs also enabled, the debuff must satisfy both rules: yours and whitelisted.\nBlacklist always wins.", 1, 1, 1, true)
                GameTooltip:Show()
            end)
            EnemyOnlyWhitelistToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
            FilterContainer:AddChild(EnemyOnlyWhitelistToggle)
        end

        PartyRaidWhitelistEditBox = AG:Create("MultiLineEditBox")
        PartyRaidWhitelistEditBox:SetLabel("Whitelist " .. auraIDName)
        PartyRaidWhitelistEditBox:SetText(AuraDB.Whitelist)
        PartyRaidWhitelistEditBox:SetNumLines(3)
        PartyRaidWhitelistEditBox:SetRelativeWidth(0.5)
        PartyRaidWhitelistEditBox:SetCallback("OnEnterPressed", function(_, _, value)
            AuraDB.Whitelist = value or ""
            UpdateAuras()
        end)
        FilterContainer:AddChild(PartyRaidWhitelistEditBox)

        PartyRaidBlacklistEditBox = AG:Create("MultiLineEditBox")
        PartyRaidBlacklistEditBox:SetLabel("Blacklist " .. auraIDName)
        PartyRaidBlacklistEditBox:SetText(AuraDB.Blacklist)
        PartyRaidBlacklistEditBox:SetNumLines(3)
        PartyRaidBlacklistEditBox:SetRelativeWidth(0.5)
        PartyRaidBlacklistEditBox:SetCallback("OnEnterPressed", function(_, _, value)
            AuraDB.Blacklist = value or ""
            UpdateAuras()
        end)
        FilterContainer:AddChild(PartyRaidBlacklistEditBox)

        if isEnemyDebuffs then
            GUIWidgets.CreateInformationTag(FilterContainer, "|cFF8080FFOnly My Debuffs|r restricts every result to your own debuffs. |cFF8080FFOnly Whitelist|r restricts results to listed IDs. With both enabled, only your whitelisted debuffs are shown. With both disabled: your debuffs + Whitelist exceptions from any source. |cFF8080FFBlacklist|r always wins.")
        else
            GUIWidgets.CreateInformationTag(FilterContainer, "|cFF8080FFBlacklist|r always wins. |cFF8080FFWhitelist|r adds listed " .. auraIDName .. " independently of Blizzard category filters.")

            CategoryFiltersToggle = AG:Create("CheckBox")
            CategoryFiltersToggle:SetLabel("Use Blizzard Category Filters")
            CategoryFiltersToggle:SetValue(AuraDB.UseBlizzardCategoryFilters == true)
            CategoryFiltersToggle:SetFullWidth(true)
            CategoryFiltersToggle:SetCallback("OnValueChanged", function(_, _, value)
                AuraDB.UseBlizzardCategoryFilters = value == true
                UpdateAuras()
                RefreshAuraGUI()
            end)
            CategoryFiltersToggle:SetCallback("OnEnter", function()
                GameTooltip:SetOwner(CategoryFiltersToggle.frame, "ANCHOR_CURSOR")
                GameTooltip:AddLine("Disabled: use the default Blizzard aura pool.\nEnabled: Player and Others dropdowns become active; empty selections include no category-filtered auras.\nWhitelist and Blacklist IDs remain independent.", 1, 1, 1, true)
                GameTooltip:Show()
            end)
            CategoryFiltersToggle:SetCallback("OnLeave", function() GameTooltip:Hide() end)
            FilterContainer:AddChild(CategoryFiltersToggle)
        end
    end

    if not (isUnifiedManagedAura and isEnemyDebuffs) then
    for _, filter in ipairs(UUF.AURA_FILTERS[filterAuraDB]) do
        if filter.Group == "General" then
            -- "Show Debuffs I Can Dispel" is intentionally not offered on Buffs.
            ...
            FilterToggle:SetRelativeWidth(isUnifiedManagedAura and 0.5 or 0.33)
            FilterToggle:SetCallback("OnValueChanged", function(_, _, value)
                AuraDB.Filters[filterKey] = value or nil
                UpdateAuras()
                RefreshAuraGUI()
            end)
            ...
            GeneralFilterToggles[#GeneralFilterToggles + 1] = FilterToggle
        end
    end

    GUIWidgets.CreateInformationTag(FilterContainer, "Dropdowns support |cFF8080FFmultiple selections|r. |cFFFFCC00Player|r is specifically you; |cFFFFCC00Others|r are all other players/units.")
    ...
    for _, filterGroup in ipairs({"Player (You)", "Others (Not You)"}) do
        ...
        if isUnifiedManagedAura then
            if filterGroup == "Player (You)" then
                filterList.Player = "All My Auras"
                filterDesc.Player = "Show every aura applied by you or your vehicle, without requiring a Blizzard category match."
            else
                filterList.Others = "All Other Auras"
                filterDesc.Others = "Show every aura applied by other players/units, without requiring a Blizzard category match."
                table.insert(filterOrder, 1, "Others")
            end
        end
        ...
        if isUnifiedManagedAura and FilterDropdown.hasClose and FilterDropdown.pullout and FilterDropdown.pullout.items then
            local closeItem = FilterDropdown.pullout.items[#FilterDropdown.pullout.items]
            if closeItem then
                FilterDropdown.pullout.items[#FilterDropdown.pullout.items] = nil
                AG:Release(closeItem)
                local itemHeight = #FilterDropdown.pullout.items * 16
                FilterDropdown.pullout.itemFrame:SetHeight(itemHeight)
                FilterDropdown.pullout.frame:SetHeight(math.min(itemHeight + 34, FilterDropdown.pullout.maxHeight))
            end
            FilterDropdown.hasClose = nil
        end
    ...
    function RefreshAuraGUI()
        if PandemicGlowContainer then
            PandemicToggle:SetDisabled(not AuraDB.Enabled)
            PandemicTypeDropdown:SetDisabled(not AuraDB.Enabled or not PandemicDB.Enabled)
            PandemicColourPicker:SetDisabled(not AuraDB.Enabled or not PandemicDB.Enabled)
        end

        if BossDebuffFilterContainer then
            OnlyMyDebuffsToggle:SetDisabled(not AuraDB.Enabled)
            OnlyWhitelistToggle:SetDisabled(not AuraDB.Enabled)
            WhitelistEditBox:SetDisabled(not AuraDB.Enabled)
            BlacklistEditBox:SetDisabled(not AuraDB.Enabled)
        end

        if AuraDB.Enabled then
            GUIWidgets.DeepDisable(AuraContainer, false, Toggle)
            if FilterContainer then GUIWidgets.DeepDisable(FilterContainer, false) end

            if FilterContainer and isUnifiedManagedAura then
                local advancedFiltersDisabled = AuraDB.UseBlizzardCategoryFilters ~= true
                if CategoryFiltersToggle then CategoryFiltersToggle:SetDisabled(false) end
                if PartyRaidWhitelistEditBox then PartyRaidWhitelistEditBox:SetDisabled(false) end
                if PartyRaidBlacklistEditBox then PartyRaidBlacklistEditBox:SetDisabled(false) end
                for _, FilterToggle in ipairs(GeneralFilterToggles) do FilterToggle:SetDisabled(advancedFiltersDisabled) end
                for _, FilterDropdown in ipairs(FilterDropdowns) do FilterDropdown:SetDisabled(advancedFiltersDisabled) end
            elseif FilterContainer then
                GUIWidgets.DeepDisable(FilterContainer, AuraDB.OnlyShowPlayer)
                for _, FilterDropdown in ipairs(FilterDropdowns) do
                    FilterDropdown:SetDisabled(AuraDB.OnlyShowPlayer or (filterAuraDB == "Debuffs" and AuraDB.Filters.Typed))
                end
            end
            ...
            if FilterContainer then GUIWidgets.DeepDisable(FilterContainer, true, Toggle) end
            GUIWidgets.DeepDisable(LayoutContainer, true, Toggle)
...
local function CreateUnitSettings(containerParent, unit)
	...
	if unit ~= "augmentation" then
		...
		-- "Enable UUF" y "Hide Blizzard" son opciones independientes.
		-- Se permite mostrar ambos frames o cualquiera de los dos.
    ...
    local function SelectUnitTab(SubContainer, _, UnitTab)
        ...
        elseif UnitTab == "Portrait" then
            CreatePortraitSettings(SubContainer, unit, function()
                UpdateUnitSettings(unit, function()
...
function UUF:CreateGUI()
    ...
    local availableWidth = math.max(900, (UIParent:GetWidth() or 1380) - 80)
    local availableHeight = math.max(520, (UIParent:GetHeight() or 800) - 80)
    Container:SetWidth(math.min(1300, availableWidth))
    Container:SetHeight(math.min(720, availableHeight))
