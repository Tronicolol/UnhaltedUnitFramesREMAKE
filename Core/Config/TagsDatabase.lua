...
local function IsUnitHealer(unit)
    ...
    -- Midnight 12.1: el rol puede ser un string secret cuando la
    -- identidad de la unidad está protegida.
    if UUF:IsSecretValue(role) then return false end
    return role == "HEALER"
end

oUF.Tags.Methods["curpp:manapercent:healer"] = function(unit)
    if not unit or not UnitExists(unit) then return "" end
    if not IsUnitHealer(unit) then return "" end
