...
-- Mantener separadas las dos decisiones:
--   Units[x].Enabled            -> frame de UnhaltedUnitFrames
--   Units[x].ForceHideBlizzard -> frame equivalente de Blizzard
--
-- oUF llama DisableBlizzard() automáticamente al hacer Spawn(); esta policy
-- hace que esa llamada respete la configuración real de UUF.
oUF.ShouldDisableBlizzard = function(_, unit)
	if not UUF.db or not UUF.db.profile or not UUF.db.profile.Units then
		return nil
	end

	local normalizedUnit = UUF:GetNormalizedUnit(unit)
	local UnitDB = normalizedUnit and UUF.db.profile.Units[normalizedUnit]

	-- nil conserva el comportamiento original de oUF para unidades que UUF
	-- no gestione explícitamente (por ejemplo nameplates de otros layouts).
	if not UnitDB then return nil end

	return UnitDB.ForceHideBlizzard == true
end
