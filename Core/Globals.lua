...
function UUF:ApplyCooldownText(icon, textRegion, unit, unitFrame, skipCountdownFormatter)
    ...
    -- AuraContainer 12.1 puede entregar CooldownFrames protegidos/forbidden.
    -- En esos botones el formatter se registra mediante AuraButton:SetDurationText()
    -- y no debemos llamar SetCountdownFormatter() sobre el CooldownFrame.
    -- El argumento es opcional para conservar intacto todo el comportamiento legacy.
    if not skipCountdownFormatter and icon.SetCountdownFormatter then
        ...
        C_Timer.After(0.01, function()
            for _, region in ipairs({icon:GetRegions()}) do
                if region:GetObjectType() == "FontString" then
					UUF:ApplyCooldownText(icon, region, unit, unitFrame, skipCountdownFormatter)
                    return
                end
            end
        end)
...
function UUF:GetUnitColour(unit)
    if UnitIsPlayer(unit) or UnitInPartyIsAI(unit) then
        ...
        -- Midnight 12.1: UnitClass() puede devolver una clase secret.
        -- No se puede usar un secret como clave de RAID_CLASS_COLORS.
...
function UUF:GetClassColour(unitFrame)
    ...
    -- Midnight 12.1: una clase secret no puede indexar tablas Lua.
    if not class or UUF:IsSecretValue(class) then return end
...
UUF.AURA_FILTERS = {
    ...
    Debuffs = {
        ...
        {Key = "RaidPlayerDispellable", Group = "General", Title = "Show Debuffs I Can Dispel", Desc = "Show debuffs Blizzard marks as dispellable by your current character."},
