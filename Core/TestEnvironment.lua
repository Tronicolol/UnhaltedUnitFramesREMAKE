...
local function ApplyTestGroupFrame(unitFrame, unit, index, displayName, element)
	...
	-- Party/Raid preview reuses live oUF frames. While preview is active, block
	-- live health/prediction event handlers from overwriting synthetic values.
	unitFrame.UUFTestModeActive = true
	...
	if (updateAll or element == "Frame" or element == "HealthBar") and unitFrame.Health then
		...
		-- Preview uses absolute synthetic HP while live optimized Health uses 0-100.
		-- Mark the range as non-live so restore cannot leave a 0-8M range behind.
		unitFrame.Health.UUFGroupPercentRange = false
	...
	if updateAll or element == "Frame" or element == "HealPrediction" then
		if unitFrame.UUFGroupUnifiedPrediction and unitFrame.Health then
			UUF:UpdateUnitHealPrediction(unitFrame, unit)
			local predictionBars = unitFrame.UUFGroupPredictionBars or {}
			SetTestPredictionBar(unitFrame.Health.DamageAbsorb, testData.absorb, testData.maxHealth, HealPredictionDB.Absorbs.Enabled)
			SetTestPredictionBar(unitFrame.Health.HealAbsorb, testData.healAbsorb, testData.maxHealth, HealPredictionDB.HealAbsorbs.Enabled)
			SetTestPredictionBar(unitFrame.Health.HealingPlayer, testData.incomingHeal, testData.maxHealth, HealPredictionDB.IncomingHeal.Enabled)
			if predictionBars.overDamageAbsorb then
				...
				SetTestPredictionBar(predictionBars.overDamageAbsorb, testData.absorb, testData.maxHealth, showOverAbsorb)
				if predictionBars.overDamageAbsorb.Clip then
					if showOverAbsorb then predictionBars.overDamageAbsorb.Clip:Show() else predictionBars.overDamageAbsorb.Clip:Hide() end
				end
			end
		elseif unitFrame.HealthPrediction then
			UUF:UpdateUnitHealPrediction(unitFrame, unit)
...
local function RestoreGroupFrame(unitFrame, unit)
	...
	unitFrame.UUFTestModeActive = false
	if unitFrame.Health and unitFrame.UUFGroupLeanHealth then
		-- Preview temporarily changes the StatusBar to absolute synthetic HP.
		-- Restore the live optimized 0-100 range before any ForceUpdate/event.
		unitFrame.Health.UUFGroupPercentRange = false
...
function UUF:EnableTestGroupFrames(unit)
	...
	if unit == "party" then
		...
		if UUF.PARTY_SOLO_CONTAINER then
			UnregisterStateDriver(UUF.PARTY_SOLO_CONTAINER, "visibility")
			UUF.PARTY_SOLO_CONTAINER:Hide()
		end
...
local function UpdateBossTestEnvironment(element)
	...
	if UUF.BOSS_TEST_MODE then
		for i, BossFrame in ipairs(UUF.BOSS_FRAMES) do
			BossFrame.UUFTestModeActive = true
			...
			if (updateAll or element == "Frame" or element == "HealthBar") and BossFrame.Health then
				...
				BossFrame.Health.UUFGroupPercentRange = false
			...
			if updateAll or element == "Frame" or element == "HealPrediction" then
				if BossFrame.UUFGroupUnifiedPrediction and BossFrame.Health then
					UUF:UpdateUnitHealPrediction(BossFrame, "boss" .. i)
					local predictionBars = BossFrame.UUFGroupPredictionBars or {}
					local maxHealth = TestData[i].maxHealth
					SetTestPredictionBar(BossFrame.Health.DamageAbsorb, TestData[i].absorb, maxHealth, HealPredictionDB.Absorbs.Enabled)
					SetTestPredictionBar(BossFrame.Health.HealAbsorb, TestData[i].healAbsorb, maxHealth, HealPredictionDB.HealAbsorbs.Enabled)
					SetTestPredictionBar(BossFrame.Health.HealingPlayer, TestData[i].incomingHeal, maxHealth, HealPredictionDB.IncomingHeal.Enabled)
					if predictionBars.overDamageAbsorb then
						local showOverAbsorb = HealPredictionDB.Absorbs.Enabled and HealPredictionDB.Absorbs.ShowOverAbsorb and HealPredictionDB.Absorbs.Position == "ATTACH"
						SetTestPredictionBar(predictionBars.overDamageAbsorb, TestData[i].absorb, maxHealth, showOverAbsorb)
						if predictionBars.overDamageAbsorb.Clip then
							if showOverAbsorb then predictionBars.overDamageAbsorb.Clip:Show() else predictionBars.overDamageAbsorb.Clip:Hide() end
						end
					end
				elseif BossFrame.HealthPrediction then
					UUF:UpdateUnitHealPrediction(BossFrame, "boss" .. i)
					local maxHealth = TestData[i].maxHealth
		...
		for i, BossFrame in ipairs(UUF.BOSS_FRAMES) do
			...
			BossFrame.UUFTestModeActive = false
			if BossFrame.Health and BossFrame.UUFGroupLeanHealth then
				BossFrame.Health.UUFGroupPercentRange = false
				BossFrame.Health:SetMinMaxValues(0, 100)
			end

			-- Desactivar oUF Auras ANTES de RegisterUnitWatch:
			-- RegisterUnitWatch puede mostrar el frame inmediatamente.
			BossFrame.Buffs = nil
			BossFrame.Debuffs = nil
			if BossFrame:IsElementEnabled("Auras") then
				BossFrame:DisableElement("Auras")
			end

			BossFrame:SetAttribute("unit", bossUnit)
			...
			if BuffsDB.Enabled or DebuffsDB.Enabled then
				UUF:UpdateUnitAuras(BossFrame, bossUnit)
				if UUF.RefreshMidnightManagedAuras then
					UUF:RefreshMidnightManagedAuras(BossFrame, bossUnit, true)
				end
			end
