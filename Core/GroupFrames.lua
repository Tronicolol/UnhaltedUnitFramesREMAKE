...
local function LayoutSoloPartyFrame()
	local UnitDB = UUF.db.profile.Units.party
	if not UnitDB or not UnitDB.Frame or not UUF.PARTY_SOLO_CONTAINER or not UUF.PARTY_SOLO_PLAYER then return end
	local FrameDB = UnitDB.Frame
	UUF.PARTY_SOLO_CONTAINER:ClearAllPoints()
	UUF.PARTY_SOLO_CONTAINER:SetPoint(FrameDB.Layout[1], UIParent, FrameDB.Layout[2], FrameDB.Layout[3], FrameDB.Layout[4])
	UUF.PARTY_SOLO_CONTAINER:SetFrameStrata(FrameDB.FrameStrata)
	UUF.PARTY_SOLO_CONTAINER:SetSize(FrameDB.Width, FrameDB.Height)
	UUF.PARTY_SOLO_PLAYER:ClearAllPoints()
	UUF.PARTY_SOLO_PLAYER:SetPoint("TOPLEFT", UUF.PARTY_SOLO_CONTAINER, "TOPLEFT", 0, 0)
	UUF.PARTY_SOLO_PLAYER:SetSize(FrameDB.Width, FrameDB.Height)
	UUF.PARTY_SOLO_PLAYER:SetFrameStrata(FrameDB.FrameStrata)
end

function UUF:SpawnSoloPartyFrame()
	local UnitDB = UUF.db.profile.Units.party
	local FrameDB = UnitDB and UnitDB.Frame
	if not UnitDB or not UnitDB.Enabled or not FrameDB or not FrameDB.ShowPlayerWhileSolo then return end
	if InCombatLockdown() then return end

	if not UUF.PARTY_SOLO_CONTAINER then
		UUF.PARTY_SOLO_CONTAINER = CreateFrame("Frame", "UUF_PartySoloContainer", UIParent, "BackdropTemplate")
		UUF.PARTY_SOLO_CONTAINER:SetBackdrop(UUF.BACKDROP)
		UUF.PARTY_SOLO_CONTAINER:SetBackdropColor(0, 0, 0, 0)
		UUF.PARTY_SOLO_CONTAINER:SetBackdropBorderColor(0, 0, 0, 0)
	end

	if not UUF.PARTY_SOLO_PLAYER then
		local activeStyle = oUF:GetActiveStyle()
		oUF:SetActiveStyle(UUF:FetchFrameName("party"))
		local soloFrame = oUF:Spawn("player", "UUF_PartySoloPlayer")
		if activeStyle then oUF:SetActiveStyle(activeStyle) end
		soloFrame:SetParent(UUF.PARTY_SOLO_CONTAINER)
		soloFrame.partyIndex = 1
		soloFrame.UUFGroupUnit = "partyplayer"
		UUF.PARTY_SOLO_PLAYER = soloFrame
		UUF:RegisterTargetGlowIndicatorFrame(soloFrame, "partyplayer")
		UUF:RegisterRangeFrame(soloFrame, "player")
		UUF:RegisterDispelHighlightEvents(soloFrame, "player")
		UUF:UpdateUnitFrame(soloFrame, "partyplayer")
	end

	LayoutSoloPartyFrame()
	RegisterStateDriver(UUF.PARTY_SOLO_CONTAINER, "visibility", "[nogroup] show; hide")
end

function UUF:UpdateSoloPartyFrame()
	local UnitDB = UUF.db.profile.Units.party
	local FrameDB = UnitDB and UnitDB.Frame
	if not UnitDB or not UnitDB.Enabled or not FrameDB or not FrameDB.ShowPlayerWhileSolo then
		if UUF.PARTY_SOLO_CONTAINER and not InCombatLockdown() then
			UnregisterStateDriver(UUF.PARTY_SOLO_CONTAINER, "visibility")
			UUF.PARTY_SOLO_CONTAINER:Hide()
		end
		return
	end
	if not UUF.PARTY_SOLO_PLAYER then UUF:SpawnSoloPartyFrame() return end
	UUF:UpdateUnitFrame(UUF.PARTY_SOLO_PLAYER, "partyplayer")
	LayoutSoloPartyFrame()
	if not InCombatLockdown() then RegisterStateDriver(UUF.PARTY_SOLO_CONTAINER, "visibility", "[nogroup] show; hide") end
end
